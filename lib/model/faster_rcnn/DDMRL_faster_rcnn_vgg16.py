...
class _da_fasterRCNN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic):
        ...
        self.D_img = MultiDomainDisc()

    def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop=None, uncertain=None, train_G=None, dc_label=None):
        ...
        if self.training: # target:0 / source:1
            DA_loss = 0.1 * self.D_img(GradReverse.apply(base_feat), dc_label)
        else:
            DA_loss_img = 0
        ...
        return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, DA_loss
...
class MultiDomainDisc(nn.Module):
    def __init__(self):
    ...
    def forward(self, feat, label):
        ...
        label = LabelResizeLayer(feat, label)
        loss = F.cross_entropy(feat, label)
        return loss


def LabelResizeLayer(feats, lbs):
